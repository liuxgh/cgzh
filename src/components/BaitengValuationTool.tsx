    ...
    <div className="space-y-6">
      ...
      <div className="bg-linear-to-r from-[#082C6C] via-[#0F52BA] to-[#0A3D8F] rounded-3xl p-6 sm:p-8 text-white shadow-xl border border-blue-400/30 relative overflow-hidden">
        <div className="relative z-10 max-w-3xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/15 text-blue-100 text-xs font-semibold mb-3 border border-white/20 backdrop-blur-xs">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {BAITENG_VALUATION_METRICS.map((metric, i) => (
          <div key={metric.key} className="bg-white p-4 rounded-2xl border border-[#D8E2F0] shadow-xs">
            <div className="text-[10px] text-[#FF7A00] font-bold uppercase tracking-wider mb-1">评估维度 {i + 1}</div>
            <div className="text-xs font-bold text-slate-900 mb-1">{metric.label.split('(')[0]}</div>
      ...
      <div className="bg-white rounded-3xl border border-[#D8E2F0] p-6 shadow-xs space-y-5">
        ...
        {!useCustom ? (
          <div className="space-y-3">
            <label className="text-xs text-slate-600 font-medium block">从吉林大学现有成果库中选择专利:</label>
            <select
              value={selectedPatentId}
              onChange={(e) => setSelectedPatentId(e.target.value)}
              className="w-full text-xs font-semibold p-2.5 bg-slate-50 border border-slate-300 rounded-xl text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-500"
            >
              {patents.map((p) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-3">
              <div>
                <label className="text-xs text-slate-600 font-medium block mb-1">成果/专利名称:</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-xs text-slate-600 font-medium block mb-1">发明人团队:</label>
                  <input
                    type="text"
                    placeholder="如：张教授团队"
                ...
                <div>
                  <label className="text-xs text-slate-600 font-medium block mb-1">所属学院/实验室:</label>
                  <input
                    type="text"
            ...
            <div>
              <label className="text-xs text-slate-600 font-medium block mb-1">技术方案与创新点摘要:</label>
              <textarea
                rows={4}
                placeholder="简要描述技术解决的行业技术瓶颈、关键工艺参数与实验室/中试测试表现..."
                value={customAbstract}
                onChange={(e) => setCustomAbstract(e.target.value)}
                className="w-full text-xs p-2.5 bg-slate-50 border border-slate-300 rounded-lg resize-none"
              />
            </div>
          </div>
        )}
        ...
        <div className="text-center pt-2">
          <button
            ...
            disabled={loading}
            className="inline-flex items-center gap-2 px-6 py-3 bg-linear-to-r from-[#FF9500] to-[#FF7A00] hover:from-[#FFA726] hover:to-[#FF8A00] text-slate-950 font-black text-xs rounded-xl shadow-md transition-all disabled:opacity-50 cursor-pointer"
          >
            {loading ? (
              <>
      ...
      {valuationResult && (
        <div className="bg-white rounded-2xl border border-amber-200 p-6 shadow-sm space-y-6 animate-in fade-in duration-200">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between pb-4 border-b border-slate-200 gap-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-slate-400 font-mono">评估标准: Q/BT-IP-2026-VAL</span>
              </div>
            ...
            <div className="flex items-center gap-4 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 px-5 py-3 rounded-xl">
              <div>
                <span className="text-[10px] text-amber-800 font-medium block">综合价值指数</span>
                <span className="text-3xl font-black text-amber-700 font-mono">{valuationResult.overallScore}</span>
              ...
              <div className="border-l border-amber-200 pl-4">
                ...
                <span className="text-lg font-bold text-blue-700 font-mono">TRL {valuationResult.trlLevel} 级</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-center">
              <div className="text-xs text-slate-500 font-medium mb-1">技术先进度评分</div>
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-center">
              <div className="text-xs text-slate-500 font-medium mb-1">法律稳定性评分</div>
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-center">
              <div className="text-xs text-slate-500 font-medium mb-1">市场应用前景评分</div>
