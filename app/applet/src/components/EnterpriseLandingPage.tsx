...
export const EnterpriseLandingPage: React.FC<Props> = ({ onSelectUniversity }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      {/* Hero Section */}
      <div className="relative overflow-hidden rounded-3xl bg-slate-900 text-white p-8 sm:p-14 shadow-xl">
        ...
        <div className="relative z-10 max-w-4xl space-y-6">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 border border-white/20 text-blue-200 text-sm font-bold">
      ...
      <div className="space-y-6 pt-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <span className="text-sm font-semibold text-slate-500">更多双一流高校持续接入中...</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {universities.map((uni) => (
            ...
            >
              {uni.status !== 'active' && (
                <div className="absolute top-6 right-6 px-2.5 py-1 bg-slate-200 text-slate-600 text-xs font-bold rounded-lg">
                  即将接入
                </div>
              )}
              {uni.status === 'active' && (
                <div className="absolute top-6 right-6 w-8 h-8 rounded-full bg-blue-50 flex items-center justify-center text-blue-600 group-hover:bg-blue-600 group-hover:text-white transition-colors">
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {uni.tags.map(tag => (
                  <span key={tag} className="px-2 py-0.5 bg-slate-100 text-slate-600 text-[11px] font-bold rounded-md">
                    {tag}
                  </span>
                ))}
              </div>
              ...
              <div className="space-y-3 pt-4 border-t border-slate-100">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-500 flex items-center gap-1.5"><Database className="w-4 h-4" /> 成果库容量</span>
                  <span className="font-bold text-slate-900">{uni.stats.patents}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-500 flex items-center gap-1.5"><Fingerprint className="w-4 h-4" /> 优势学科</span>
